...
public final class QRCodeUtil extends LuminanceSource {
    ...
    public static void generateQRCode(String text, int width, int height, String format, HttpServletResponse response)
        ...
        hints.put(EncodeHintType.CHARACTER_SET, "utf-8");// 指定编码格式
//        hints.put(EncodeHintType.ERROR_CORRECTION, ErrorCorrectionLevel.L);// 指定纠错等级
        hints.put(EncodeHintType.ERROR_CORRECTION, ERROR_CORRECTION_List.get(random.nextInt(ERROR_CORRECTION_List.size())));// 指定纠错等级
        hints.put(EncodeHintType.MARGIN, 1); // 白边大小，取值范围0~4
        BitMatrix bitMatrix = new MultiFormatWriter().encode(text, BarcodeFormat.QR_CODE, width, height, hints);
    ...
    final static ArrayList ERROR_CORRECTION_List = new ArrayList(){{
        add(ErrorCorrectionLevel.L);
        add(ErrorCorrectionLevel.H);
        add(ErrorCorrectionLevel.M);
//        add(ErrorCorrectionLevel.Q);
    }};
    ...
    public static void main(String[] args) {
        ...
        try {
            // 生成二维码图片，并返回图片路径
            String pathName = generateQRCode(text, width, height, format, "F:/zPythonTest/new.png");
